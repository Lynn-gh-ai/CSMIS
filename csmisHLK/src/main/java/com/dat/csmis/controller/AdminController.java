...
public class AdminController {
	@Autowired
	DateListEntityService dateListService;
	@Autowired
	...
	public String doRestaurant(@ModelAttribute("res") RestaurantModel rm, @RequestParam("datPrice") Double datprice,
		...
		if (restaurantRepository.existsByEmail(rm.getEmail()) && restaurantRepository.existsByPhone(rm.getPhone())) {

			model.addAttribute("errorS", "Email and phone is duplicate");
			return "adminRestaurant";

		} else if (restaurantRepository.existsByEmail(rm.getEmail())) {
	...
	public String updateRestaurant(@ModelAttribute("res") RestaurantEntity rm,
		...
		if (rm != null) {
			...
			System.out.println("the id is ==> " + re.getId());
			entity.setId(rm.getId());
			serviceR.save(entity, re.getId());
	...
	public String doProfile(@RequestParam("propic") MultipartFile part, @RequestParam("sid") String staffid,
			@RequestParam("userName") String name, @RequestParam("opsw") String opsw, @RequestParam("npsw") String npsw,
			RedirectAttributes m, HttpServletRequest req) {
	...
	public String updateRes(@PathVariable("id") int id, Model m) {
		...
		if (op.isPresent() && op2.isPresent()) {
			re = op.get();
			pe = op2.get();
			m.addAttribute("dat", pe.getDatPrice());
			m.addAttribute("emp", pe.getEmpPrice());
	...
	public String setStatus(@PathVariable("id") int id, @PathVariable("status") String status) {
	...
	public String doorFile(@ModelAttribute("orderList") OrderList orderList, Model model,
	...
	public String findPagination(@PathVariable(value = "pageNo") int pageNo,
		...
		if (start != null && end != null && !start.isEmpty() && !end.isEmpty()) {
			if (isStartDateAfterEndDate(start, end)) {
				...
				|| (start == null && end != null && start.isEmpty() && !end.isEmpty())) {
			model.addAttribute("error", "Please enter start date and end date");
			return "WeeklyOrderList";
		}
		Page<OrderListEntity> page = orderListService.findPagination(pageNo, pageSize, sortField, sortDir, start, end,
				status);
...
//		}
	...
	public String displayPaymentConfirm(@PathVariable("id") Integer id, @RequestParam("vId") String vid,
			@ModelAttribute("payment") PaymentCashierBean payment, Model model, RedirectAttributes ra) {
		Optional<OrderListEntity> orderDetail = orderListService.getFindOne(id);
		if (orderDetail.isPresent()) {
...
////////////////////////////////////////////////////////////////DoorLog Start ///////////////////////////////////////////////////////////////////
	...
	@GetMapping("/admin/re")
	public String reList(HttpServletRequest request, @RequestParam("selectedDate") String selectedDate, Model model) {
		var listOfdate = doorlogService.selectDate();
	...
	public String handleFormSubmit(@RequestParam(name = "submitBtn", required = false) String submitBtn,
		...
		if (submitBtn != null) {
			...
			Integer reCount = lvRepo.findRE(outputDate);
			System.out.print(outputDate);
			System.out.print(reCount);
			model.addAttribute("reCount", reCount);
			model.addAttribute("date", outputDate);
			model.addAttribute("dateOfArray", listOfdate);
		}
	...
	public String rneList(HttpServletRequest request, @RequestParam("selectedDate") String selectedDate, Model model) {
	...
	public String handleFormSubmit2(@RequestParam(name = "submitBtn", required = false) String submitBtn,
		...
		if (submitBtn != null) {
			...
			Integer rneCount = lvRepo.findRNE(outputDate);
			System.out.print(outputDate);
			System.out.print(rneCount);
			model.addAttribute("rneCount", rneCount);
			model.addAttribute("rnedate", outputDate);
			model.addAttribute("dateOfArray", listOfdate);
		}
	...
	@GetMapping("/admin/ue")
	public String ueList(HttpServletRequest request, @RequestParam("selectedDate") String selectedDate, Model model) {
	...
	public String handleFormSubmit3(@RequestParam(name = "submitBtn", required = false) String submitBtn,
		...
		if (submitBtn != null) {
			...
			Integer ueCount = lvRepo.findUE(outputDate);
			System.out.print(outputDate);
			System.out.println("jkdfkdf :: " + (lvRepo.findUE(outputDate) == null));
			System.out.print(ueCount);
			model.addAttribute("ueCount", ueCount);
			model.addAttribute("uedate", outputDate);
			model.addAttribute("dateOfArray", listOfdate);
		}
	...
	public String handleFormSubmit4(@RequestParam(name = "submitBtn", required = false) String submitBtn,
			@RequestParam("totaldate") String selectedDate, Model model) {
		var listOfdate = doorlogService.selectDate();
		...
		if (submitBtn != null) {
			...
			model.addAttribute("totaldate", outputDate);
			model.addAttribute("dateOfArray", listOfdate);
		}
		return "admindoorlogcard";
	}
...
//	}
	...
	public String findLunchAllPagination(@PathVariable(value = "pageNo") int pageNo,
			@RequestParam("sortField") String sortField, @RequestParam("sortDir") String sortDir,
			@RequestParam(value = "start", required = false) String start,
			@RequestParam(value = "end", required = false) String end,
			@RequestParam(value = "status", required = false) String status, Model model) {
		...
		Page<DoorLogAllViewEntity> page = doorlogallViewSer.findLunchAllPagination(pageNo, pageSize, sortField, sortDir,
				start, end, status);
	...
	public void generateReportDoorlog(HttpServletResponse response, @RequestParam("type") String type,
			@RequestParam(value = "start", required = false) String start,
			@RequestParam(value = "end", required = false) String end,
			@RequestParam(value = "status", required = false) String reg) throws Exception {
		System.out.println("start--------" + start);
	...
	public ResponseEntity<String> updateAvoidMeat(@PathVariable("id") Integer id) {
		...
		try {
			...
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error retrieving Avoid Meat");
		}
	}
	...
	public ResponseEntity<EmployeeEntity> updateEmployeeList(@PathVariable("id") Long id) {
		...
		try {
			...
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null); // Error retrieving Employee
		}
	}

	@PostMapping("/admin/seyupUpdateEmployee")

	public ResponseEntity<String> setupUpdateEmployee(@ModelAttribute("employeeData") EmployeeEntity employee) {
	...
	public String saveHoliday(@RequestParam("date") String date, @RequestParam("holidays") String holidays,
	...
	public String searchCost(@PathVariable(value = "pageNo") int pageNo, @RequestParam("sortField") String sortField,
			@RequestParam("sortDir") String sortDir, @RequestParam(value = "start", required = false) String start,
			@RequestParam(value = "end", required = false) String end, Model m) {
		...
		Page<WeeklyOrderEntity> page = weeklyOrderService.findPagination(pageNo, pageSize, sortField, sortDir, start,
				end);
		...
		for (WeeklyOrderEntity week : b) {
			...
			if (view.isPresent()) {
				...
				int count = entity.getRe() + entity.getRne() + entity.getUe();
				cc.setEmpCostCount(count);
			}

			cc.setDate(week.getDate());
			cc.setDatPrice(week.getOrderList().getRestaurantEntity().getPrice().getDatPrice());
			cc.setPrice(week.getPrice());
			cc.setTotalPeople(week.getTotalPeople());
			cc.setResId(week.getOrderList().getRestaurantEntity().getId());
			cc.setResName(week.getOrderList().getRestaurantEntity().getName());
			cc.setEmpPrice(week.getOrderList().getRestaurantEntity().getPrice().getEmpPrice());
			cost.add(cc);
		}
	...
	public void costReport(HttpServletResponse response, @RequestParam("type") String type,
		...
		for (WeeklyOrderEntity week : list) {
			...
			cost.setDatPrice(week.getOrderList().getRestaurantEntity().getPrice().getDatPrice());
			cost.setEmpPrice(week.getOrderList().getRestaurantEntity().getPrice().getEmpPrice());
			cost.setDatTotalPrice(
					week.getTotalPeople() * week.getOrderList().getRestaurantEntity().getPrice().getDatPrice());
	...
	public String setEmployeeStatus(@PathVariable("id") long id, @PathVariable("status") String status,
			@PathVariable("profileId") long pid, Model m) {
	...
	public String setEmployeeRole(@PathVariable("id") long id, @PathVariable("role") String role,
			@PathVariable("profileId") long pid, Model m) {
	...
	public ResponseEntity<String> doUpdateHolidays(HttpServletRequest req, @RequestParam("id") long id,
			@RequestParam("date") String date, @RequestParam("holidays") String holidays) {
		Optional<HolidaysEntity> holi = hdService.findById(id);
